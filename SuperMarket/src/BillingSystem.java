...
public class BillingSystem {
    ...
    public static void printBill_Items(ItemData li, int[] other_code, int[] other_quantity, int[] grocery_code,
            int[] grocery_quantity, int[] veg_code, int[] veg_quantity, int[] fruits_code, int[] fruits_quantity) {
        System.out.println("==============================================================================");
        System.out.println("\t Item Name \t\t\t\t Price \t\t\t Total");
        System.out.println("==============================================================================");
        for (int i = 0; i < other_code.length; i++) {
            System.out.print(getItemNameOthers(li, other_code[i]) + "\t\t\t\t " + other_quantity[i] + "*"
                    + getOtherPrice(li, other_code[i]) + "\t\t\t"
                    + getOtherPrice(li, other_code[i]) * other_quantity[i]);
            System.out.println();
        }

        for (int i = 0; i < grocery_code.length; i++) {
            System.out.print(getItemNameGrocery(li, grocery_code[i]) + "\t\t\t\t " + grocery_quantity[i] + "*"
                    + getGroceryPrice(li, grocery_code[i]) + "\t\t\t"
                    + getGroceryPrice(li, grocery_code[i]) * grocery_quantity[i]);
            System.out.println();
        }

        for (int i = 0; i < veg_code.length; i++) {
            System.out.print(getItemNameVeg(li, veg_code[i]) + "\t\t\t\t " + veg_quantity[i] + "*"
                    + getVegPrice(li, veg_code[i]) + "\t\t\t" + getVegPrice(li, veg_code[i]) * veg_quantity[i]);
            System.out.println();
        }

        for (int i = 0; i < fruits_code.length; i++) {
            System.out.print(getItemNameFruits(li, fruits_code[i]) + "\t\t\t\t " + fruits_quantity[i] + "*"
                    + getFruitsPrice(li, fruits_code[i]) + "\t\t\t"
                    + getFruitsPrice(li, fruits_code[i]) * fruits_quantity[i]);
            System.out.println();
        }
        System.out.println("==============================================================================");
    }
    ...
    public static void printBill_Details(Operator[] array, String name, String mob_no, String username,
            String password) {
    ...
    public static void main(String[] args) {
        try (Scanner in = new Scanner(System.in)) {
            ...
            operators[0] = new Operator("Ramanjit Singh", "Punjab", "raman_22", "raman22", "9461012345");
            operators[1] = new Operator("Sourav Kumar", "Punjab", "sourav01", "sourav1", "9765212345");
            operators[2] = new Operator("Ritik Katiyar", "Uttar Pradesh", "ritik02", "ritik2", "9636912345");
            ...
            Exit: while (true) {
                ...
                switch (choice_1) {
                    case 1:
                        Exit_Admin: while (attempt_admin != 0) {
                            String usr, pass;
                        ...
                        break Exit;
                    case 2:
                        Exit_Operator: while (attempt_op != 0) {
                            String usr1, pass1;
                            System.out.print("Please enter Username : ");
                            usr1 = in.nextLine();
                            System.out.print("Please enter Password : ");
                            ...
                            if (checkOperator(operators, usr1, pass1)) {
                                ...
                                if (choice_3 == 1) {
                                    ...
                                    bill = bill + billOthers(list, items_other, items_arr_others,
                                            items_arr_others_quantity);
                                    ...
                                    bill = bill + billGrocery(list, items_grocery, items_arr_grocery,
                                            items_arr_grocery_quantity);
                                    ...
                                    bill = bill + billFruits(list, items_fruits, items_arr_fruits,
                                            items_arr_fruits_quantity);

                                    printBill_Details(operators, name_cust, mob_no_cust, usr1, pass1);
                                    printBill_Items(list, items_arr_others, items_arr_others_quantity,
                                            items_arr_grocery, items_arr_grocery_quantity, items_arr_veg,
                                            items_arr_veg_quantity, items_arr_fruits, items_arr_fruits_quantity);
